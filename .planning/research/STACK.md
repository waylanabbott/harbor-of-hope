...
**Domain:** Nonprofit case management + donor platform (Harbor of Hope)
...
npm install react-icons react-cookie-consent js-cookie
...
const theme = createTheme({
  palette: {
    primary: { main: '#E8735A' },        // Coral
    secondary: { main: '#5B8C7A' },       // Sage green (complementary)
    background: {
  ...
  components: {
    MuiCard: {
      styleOverrides: {
        root: { boxShadow: '0 2px 12px rgba(0,0,0,0.08)' },  // Soft shadows
      },
    },
    MuiButton: {
      styleOverrides: {
        root: { borderRadius: 24 },       // Extra-rounded buttons
      },
    },
  },
});
...
```typescript
// Use js-cookie to persist dark mode preference
import Cookies from 'js-cookie';

const savedMode = Cookies.get('darkMode') === 'true' ? 'dark' : 'light';

// MUI supports dark mode via colorSchemes or manual palette swap
const theme = createTheme({
  palette: {
    mode: savedMode,
...
```csharp
// Replace UseSqlite with UseNpgsql in Program.cs
// Reference project pattern, adapted:
...
// Everything else from reference project stays the same:
builder.Services.AddIdentityApiEndpoints<ApplicationUser>()
    .AddRoles<IdentityRole>()
    .AddEntityFrameworkStores<AuthDbContext>();

// Password policy (from IS 414 spec):
...
```csharp
// DataSeeder.cs — runs on first startup
public static async Task SeedFromCsvAsync(HarborDbContext context)
{
    if (await context.Safehouses.AnyAsync()) return; // Already seeded

    using var reader = new StreamReader("data/lighthouse_csv_v7/safehouses.csv");
    using var csv = new CsvReader(reader, CultureInfo.InvariantCulture);
    var records = csv.GetRecords<Safehouse>().ToList();
    context.Safehouses.AddRange(records);
    await context.SaveChangesAsync();
    // Repeat for all 17 tables in dependency order
}
```
...
app = Flask(__name__)
CORS(app, origins=["https://your-azure-domain.azurewebsites.net"])

# Load all models once at startup
models = {
    "donor_churn": joblib.load("models/donor_churn.pkl"),
    "social_media": joblib.load("models/social_media.pkl"),
    "reintegration": joblib.load("models/reintegration.pkl"),
    # ... all 8 models
}
...
├── App Service (Linux, .NET 10)     → Backend API + serves React build
│   ├── React build output (wwwroot/) → Static SPA files
│   └── .NET Web API                  → /api/* routes
├── App Service (Linux, Python 3.12)  → Flask ML API
│   └── gunicorn wsgi:app             → Serves /predict/* endpoints
└── Azure Database for PostgreSQL (Flexible Server)
    ├── harbor_db                     → Application data (17 tables)
    └── identity_db                   → ASP.NET Identity tables
```
...
---
*Stack research for: Harbor of Hope nonprofit case management + donor platform*
*Researched: 2026-04-06*
